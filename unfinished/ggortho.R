...
ggortho <- function(centre.lat, centre.lon,
  ...
  suppressMessages({
    ...
    visible.lat.lines <- st_intersection(st_make_valid(lat.lines),
  ...
  p <- ggplot() +
    ...
    geom_sf(data = visible.lat.lines, colour = lon.lat.colour, linetype = lon.lat.linetype) +
    geom_sf(data = visible.lon.lines, colour = lon.lat.colour, linetype = lon.lat.linetype) +
    geom_sf_label(data = lat.ax, aes(label = lat.ax.labs),
                  label.size = 0, alpha = 0.75, label.r = unit(0.5, "lines")) +
    geom_sf_label(data = long.ax, aes(label = labs),
                  label.size = 0, alpha = 0.75, label.r = unit(0.5, "lines"))
...
test.dat <-
  data.frame(lat = c(85), lon = c(121))

test.dat.sf <- sf::st_as_sf(test.dat, coords = c("lon", "lat")) %>%
  sf::st_set_crs(4326)

p + geom_sf(data = test.dat.sf, colour = "Red")



ggortho(centre.lat = -30, centre.lon = 30,
        lat.ax.vals = seq(-90, 90, 30),
        #lat.ax.labs.pos = 60,
        plot.visible = FALSE)
