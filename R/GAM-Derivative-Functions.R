...
## From https://gist.github.com/gavinsimpson/e73f011fdaaab4bb5a30
################################################
#' Deriv
#'
#' @param mod A fitted GAM or GAMM
#' @param n The number of points at which to evaluate the derivative, defaults to 200
#' @param eps The finite difference in x (delta) over which to estimate change in y
#' @param newdata A dataframe of points (+ levels of any factors in the model) at which to evaluate the derivatives
#' @param term The term in the GAM(M) over which which derivatives should be estimated
#'
#' @return
#' @export Deriv
...
Deriv <- function(mod, n = 200, eps = 1e-7, newdata, term) {
  ...
  newDF[m.terms] <- newDF[m.terms] + eps # add eps only to model terms
  ...
  lD$gamModel <- mod
  lD$eps <- eps
  newD[m.terms] <- newD[m.terms] - eps # add eps only to model terms
  lD$eval <- newD
  lD ##return
}
