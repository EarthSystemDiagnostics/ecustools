...
##' Laepple, Thomas, and Gerrit Lohmann. 2009. “Seasonal Cycle as Template for Climate Variability on Astronomical Timescales.”
##' Paleoceanography 24 (4): PA4201. doi:10.1029/2008PA001674.
##' @title Estimate linear or polynomial transfer function between insolation and a climate parameter
...
##' coeff coefficient of polynomial, coeff.lin = coefficient of linear fit, rsq of linear and of polynomial fit; timelag in days
...
##' @export
EstimateTransferfunctionInsolation<-function(climate,latitude,polynom.order=3,clab="",bPlot=FALSE,b3plot=FALSE,main="")
{
	...
	for (iopt in 1:365) climate.matrix[,iopt]<-basis[iopt:(iopt+364)]  #save lagged versions of the climate vector

        if (polynom.order==3)
            {
                t<-lm(climate.matrix~ins.1+ins.2+ins.3)
                
                #ul, il und index.neg wurde verwendet um eine positive Steigung zu erreichen, nur in den Tropen wichtig
	...
	rsq2<-cor(climate.matrix[,bestfit.index],t$fitted.values[,bestfit.index])^2
...
{
	at<-basis
	at<-filter(at,rep(1/20,20),circular=T)
	at.scale<-scale(at)
...
if (b3plot)
{
	...
	plot(ins.1,climate.matrix[,bestfit.index],xlab="insolation (W/m²)",ylab=clab,main=main,ylim=range(climate.matrix[,bestfit.index])+c(-add,add))
	...
	return(list(coeff=coeff,coeff.lin=coeff.lin,rsq=c(rsq1,rsq2),lag=bestfit.index))
