...
##' Insolation calculation, converted and adapted from the Eisenmann and Huybers,2006
...
##' @title  Computes daily average insolation as a function of day and latitude given the orbital parameters
...
##' @author Thomas Laepple
##' @export
DailyInsolationParam<-function(lat,day,ecc,obliquity,long_perh,day_type=1,alpha=0,T.alpha=80)
{
...
##' for details see daily_insolation_param
...
##' @author Thomas Laepple
##' @examples
##' plot(0:1000,AnnualInsolation(0:1000,40),main="mean annual insolation at 40degN",xlab="kyr BP",ylab="W/m2")
##' @export
AnnualInsolation<-function(kyear,lat)
{
    result<-vector()
    for (i in 1:length(kyear)) result[i]<-mean(DailyInsolation(kyear[i],lat,1:365)$Fsw)
    return(result)
}
...
##' much slower than the day_type=1 which uses a trigeometric approximation
##' of Kepler's equation
##' @title Computes the daily average insolation for any time in the last 5Million years
##' @param kyear  Thousands of years before present (0 to 5000).
##' @param lat  Latitude in degrees (-90 to 90).
##' @param day  Indicator of time of year; calendar day by default.
...
kyear0<- -1*orbit91[,1] # kyears before present for data (kyear0>=0);
ecc0<-orbit91[,2] # eccentricity

# add 180 degrees to omega (see lambda definition, Berger 1978 Appendix)
omega0<-orbit91[,3]+180; # longitude of perihelion (precession angle)
